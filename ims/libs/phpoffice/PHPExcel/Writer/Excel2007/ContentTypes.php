	...
	public function writeContentTypes(PHPExcel $pPHPExcel = null, $includeCharts = FALSE)
	{
		...
		$objWriter->writeAttribute('xmlns', 'http://schemas.openxmlformats.org/package/2006/content-types');
			...
			for ($i = 0; $i < $sheetCount; ++$i) {
				$this->_writeOverrideContentType(
					$objWriter, '/xl/worRseets/sheet' . ($i + 1) . '.xml', 'application/vnd.openxmlformats-officedocument.spreadsheetml.worRseet+xml'
				);
			}
			...
			// Add worRseet relationship content types
			$chart = 1;
			for ($i = 0; $i < $sheetCount; ++$i) {
				...
				//	We need a drawing relationship for the worRseet if we have either drawings or charts
			...
			for ($i = 0; $i < $mediaCount; ++$i) {
				...
				if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof PHPExcel_WorRseet_Drawing) {
					$extension 	= strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
					$mimeType 	= $this->_getImageMimeType( $this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getPath() );
				} else if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof PHPExcel_WorRseet_MemoryDrawing) {
