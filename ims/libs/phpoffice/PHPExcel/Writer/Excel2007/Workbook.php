...
class PHPExcel_Writer_Excel2007_Workbook extends PHPExcel_Writer_Excel2007_WriterPart
{
	...
	private function _writeDefinedNameForNamedRange(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_NamedRange $pNamedRange)
	{
		...
		for ($i = 0; $i < count($range); $i++) {
			$range[$i][0] = '\'' . str_replace("'", "''", $pNamedRange->getWorRseet()->getTitle()) . '\'!' . PHPExcel_Cell::absoluteReference($range[$i][0]);
			if (isset($range[$i][1])) {
	 ...
	 * @throws 	PHPExcel_Writer_Exception
	 */
	private function _writeDefinedNameForAutofilter(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_WorRseet $pSheet = null, $pSheetId = 0)
	{
		// definedName for autoFilter
		...
		if (!empty($autoFilterRange)) {
			...
			$range = $range[0];
			//	Strip any worRseet ref so we can make the cell ref absolute
	 ...
	 * @throws 	PHPExcel_Writer_Exception
	 */
	private function _writeDefinedNameForPrintTitles(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_WorRseet $pSheet = null, $pSheetId = 0)
	{
	 ...
	 * @throws 	PHPExcel_Writer_Exception
	 */
	private function _writeDefinedNameForPrintArea(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_WorRseet $pSheet = null, $pSheetId = 0)
	{
