...
// Original file header of PEAR::Spreadsheet_Excel_Writer_WorRseet (used as the base for this class):
// -----------------------------------------------------------------------------------------
// /*
...
/**
 * PHPExcel_Writer_Excel5_WorRseet
 *
 * @category   PHPExcel
 * @package    PHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Writer_Excel5_WorRseet extends PHPExcel_Writer_Excel5_BIFFwriter
{
	/**
	 * Formula parser
	 *
	...
	/**
	 * Array containing the selected area for the worRseet
	 * @var array
	 */
	public $_selection;

	/**
	 * The active pane for the worRseet
	 * @var integer
	 */
	public $_active_pane;
	 ...
	 * @param string	$phpSheet		The worRseet to write
	 * @param PHPExcel_WorRseet $phpSheet
	 */
...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
	...
	private function _writeWindow2()
	{
		...
		$fPaged		 = 1;					 // 2
		$fPageBreakPreview = $this->_phpSheet->getSheetView()->getView() === PHPExcel_WorRseet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW;
	 ...
	 * references in a worRseet.
	 *
	 * Excel only stores references to external sheets that are used in formulas.
	 * For simplicity we store references to all the sheets in the workbook
	 * regardless of whether they are used or not. This reduces the overall
	 * complexity and eliminates the need for a two way dialogue between the formula
	 * parser the worRseet objects.
	 *
	 * @param integer $count The number of external sheet references in this worRseet
	 */
	 ...
	 * reference to all of the external worRseets the EXTERNSHEET index is the same
	 * as the worRseet index.
	 *
	 * @param string $sheetname The name of a external worRseet
	 */
	...
	private function _writeSetup()
	{
		...
		// Page orientation
		$fLandscape = ($this->_phpSheet->getPageSetup()->getOrientation() == PHPExcel_WorRseet_PageSetup::ORIENTATION_LANDSCAPE) ?
			0x0 : 0x1;
	...
	private function _writeBreaks()
	{
		...
		foreach ($this->_phpSheet->getBreaks() as $cell => $breakType) {
			...
			switch ($breakType) {
				case PHPExcel_WorRseet::BREAK_COLUMN:
					// Add to list of vertical breaks
					$vbreaks[] = PHPExcel_Cell::columnIndexFromString($coordinates[0]) - 1;
					break;

				case PHPExcel_WorRseet::BREAK_ROW:
					// Add to list of horizontal breaks
					$hbreaks[] = $coordinates[1];
					break;

				case PHPExcel_WorRseet::BREAK_NONE:
				default:
					// Nothing to do
					break;
			}
	...
	/**
	 * Set the Biff PROTECT record to indicate that the worRseet is protected.
	 */
	...
	/**
	 * Write the worRseet PASSWORD record.
	 */
	...
	/**
	 * Insert a 24bit bitmap image in a worRseet.
	 *
	 * @access public
