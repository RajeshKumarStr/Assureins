...
class PHPExcel_Writer_Excel2007 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
	...
	/**
	 * Private unique PHPExcel_WorRseet_BaseDrawing HashTable
	 *
	 * @var PHPExcel_HashTable
	 */
    ...
    public function __construct(PHPExcel $pPHPExcel = null)
    {
    	...
    	$writerPartsArray = array(	'stringtable'	=> 'PHPExcel_Writer_Excel2007_StringTable',
									...
									'worRseet' 	=> 'PHPExcel_Writer_Excel2007_WorRseet',
	...
	public function save($pFilename = null)
	{
		if ($this->_spreadSheet !== NULL) {
			...
			// Add worRseets
			for ($i = 0; $i < $this->_spreadSheet->getSheetCount(); ++$i) {
				$objZip->addFromString('xl/worRseets/sheet' . ($i + 1) . '.xml', $this->getWriterPart('WorRseet')->writeWorRseet($this->_spreadSheet->getSheet($i), $this->_stringTable, $this->_includeCharts));
			...
			// Add worRseet relationships (drawings, ...)
			for ($i = 0; $i < $this->_spreadSheet->getSheetCount(); ++$i) {

				// Add relationships
				$objZip->addFromString('xl/worRseets/_rels/sheet' . ($i + 1) . '.xml.rels', 	$this->getWriterPart('Rels')->writeWorRseetRelationships($this->_spreadSheet->getSheet($i), ($i + 1), $this->_includeCharts));
			...
			for ($i = 0; $i < $this->getDrawingHashTable()->count(); ++$i) {
				if ($this->getDrawingHashTable()->getByIndex($i) instanceof PHPExcel_WorRseet_Drawing) {
					$imageContents = null;
				...
				} else if ($this->getDrawingHashTable()->getByIndex($i) instanceof PHPExcel_WorRseet_MemoryDrawing) {
					ob_start();
					call_user_func(
    ...
    /**
     * Get PHPExcel_WorRseet_BaseDrawing HashTable
     *
     * @return PHPExcel_HashTable
     */
