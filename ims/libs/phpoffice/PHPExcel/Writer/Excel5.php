...
class PHPExcel_Writer_Excel5 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
	...
	public function save($pFilename = null) {
		...
		// Initialise worRseet writers
		$countSheets = $this->_phpExcel->getSheetCount();
		for ($i = 0; $i < $countSheets; ++$i) {
			$this->_writerWorRseets[$i] = new PHPExcel_Writer_Excel5_WorRseet($this->_str_total, $this->_str_unique,
		...
		for ($i = 0; $i < $countSheets; ++$i) {
			foreach ($this->_writerWorRseets[$i]->_phpSheet->getCellCollection() as $cellID) {
				$cell = $this->_writerWorRseets[$i]->_phpSheet->getCell($cellID);
				$cVal = $cell->getValue();
				if ($cVal instanceof PHPExcel_RichText) {
					...
					foreach ($elements as $element) {
						if ($element instanceof PHPExcel_RichText_Run) {
							...
							$this->_writerWorRseets[$i]->_fntHashIndex[$font->getHashCode()] = $this->_writerWorkbook->_addFont($font);
						}
					}
				}
			}
		...
		// Write the worRseet streams before the global workbook stream,
		// because the byte sizes of these are needed in the global workbook stream
		$worRseetSizes = array();
		for ($i = 0; $i < $countSheets; ++$i) {
			$this->_writerWorRseets[$i]->close();
			$worRseetSizes[] = $this->_writerWorRseets[$i]->_datasize;
		}

		// add binary data for global workbook stream
		$OLE->append($this->_writerWorkbook->writeWorkbook($worRseetSizes));

		// add binary data for sheet streams
		for ($i = 0; $i < $countSheets; ++$i) {
			$OLE->append($this->_writerWorRseets[$i]->getData());
		}
	...
	/**
	 * Build the WorRseet Escher objects
	 *
	 */
	private function _buildWorRseetEschers()
	{
		...
		foreach ($this->_phpExcel->getAllsheets() as $sheet) {
			...
			$this->_writerWorRseets[$sheetIndex]->setEscher($escher);
		}
	}
	...
	private function _buildWorkbookEscher()
	{
		...
		foreach ($this->_phpExcel->getAllsheets() as $sheet) {
			foreach ($sheet->getDrawingCollection() as $drawing) {
				if ($drawing instanceof PHPExcel_WorRseet_Drawing) {
				...
				} else if ($drawing instanceof PHPExcel_WorRseet_MemoryDrawing) {

					switch ($drawing->getRenderingFunction()) {

					case PHPExcel_WorRseet_MemoryDrawing::RENDERING_JPEG:
						$blipType = PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
						$renderingFunction = 'imagejpeg';
						break;

					case PHPExcel_WorRseet_MemoryDrawing::RENDERING_GIF:
					case PHPExcel_WorRseet_MemoryDrawing::RENDERING_PNG:
					case PHPExcel_WorRseet_MemoryDrawing::RENDERING_DEFAULT:
						$blipType = PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
