...
//		}
	...
	/**
	 * Reads names of the worRseets from a file, without parsing the whole file to a PHPExcel object
	 *
	 * @param 	string 		$pFilename
	 * @throws 	PHPExcel_Reader_Exception
	 */
	public function listWorRseetNames($pFilename)
	{
		// Check if file exists
		if (!file_exists($pFilename)) {
			throw new PHPExcel_Reader_Exception("Could not open " . $pFilename . " for reading! File does not exist.");
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					if ($xmlWorkbook->sheets) {
						foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
							...
							$worRseetNames[] = (string) $eleSheet["name"];
						}
					}
			}
		}
		...
		return $worRseetNames;
	}


	/**
	 * Return worRseet info (Name, Last Column Letter, Last Column Index, Total Rows, Total Columns)
	 *
	 * @param   string     $pFilename
	 * @throws   PHPExcel_Reader_Exception
	 */
	public function listWorRseetInfo($pFilename)
	{
		// Check if file exists
		if (!file_exists($pFilename)) {
			throw new PHPExcel_Reader_Exception("Could not open " . $pFilename . " for reading! File does not exist.");
		...
		foreach ($rels->Relationship as $rel) {
			if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
				...
				foreach ($relsWorkbook->Relationship as $ele) {
					if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/worRseet") {
						$worRseets[(string) $ele["Id"]] = $ele["Target"];
					}
				}
				...
				if ($xmlWorkbook->sheets) {
					...
					foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
						$tmpInfo = array(
							'worRseetName' => (string) $eleSheet["name"],
							'lastColumnLetter' => 'A',
							'lastColumnIndex' => 0,
							'totalRows' => 0,
							'totalColumns' => 0,
						...
						$fileWorRseet = $worRseets[(string) self::array_item($eleSheet->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "id")];

						$xml = new XMLReader();
						$res = $xml->xml($this->securityScanFile('zip://'.PHPExcel_Shared_File::realpath($pFilename).'#'."$dir/$fileWorRseet"), null, PHPExcel_Settings::getLibXmlLoaderOptions());
		...
		foreach ($rels->Relationship as $rel) {
			switch ($rel["Type"]) {
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					foreach ($relsWorkbook->Relationship as $ele) {
						switch($ele['Type']){
						case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/worRseet":
							$worRseets[(string) $ele["Id"]] = $ele["Target"];
							break;
					...
					if ($xmlWorkbook->sheets) {
						foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
							...
							//	Use false for $updateFormulaCellReferences to prevent adjustment of worRseet
							//		references in formula cells... during the load, all formulae should be correct,
							//		and we're simply bringing the worRseet name in line with the formula, not the
							//		reverse
							$docSheet->setTitle((string) $eleSheet["name"],false);
							$fileWorRseet = $worRseets[(string) self::array_item($eleSheet->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "id")];
							$xmlSheet = simplexml_load_string($this->securityScan($this->_getFromZipArchive($zip, "$dir/$fileWorRseet")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
	...
	//									echo 'Value is ', $value, PHP_EOL;
                                ...
                                if (strpos($autoFilterRange, ':') !== false) {
                                    ...
                                    foreach ($xmlSheet->autoFilter->filterColumn as $filterColumn) {
                                        ...
                                        if ($filterColumn->filters) {
                                            $column->setFilterType(PHPExcel_WorRseet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_FILTER);
                                            ...
                                            if ((isset($filters["blank"])) && ($filters["blank"] == 1)) {
                                                ...
                                                ->setRuleType(PHPExcel_WorRseet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_FILTER);
                                            }
                                            ...
                                            foreach ($filters->dateGroupItem as $dateGroupItem) {
                                                ...
                                                ->setRuleType(PHPExcel_WorRseet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DATEGROUP);
                                            }
                                        }
                                        //	Check for custom filters
                                        if ($filterColumn->customFilters) {
                                            $column->setFilterType(PHPExcel_WorRseet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_CUSTOMFILTER);
                                            $customFilters = $filterColumn->customFilters;
                                            //	Custom filters can an AND or an OR join;
                                            //		and there should only ever be one or two entries
                                            if ((isset($customFilters["and"])) && ($customFilters["and"] == 1)) {
                                                $column->setJoin(PHPExcel_WorRseet_AutoFilter_Column::AUTOFILTER_COLUMN_JOIN_AND);
                                            }
                                            foreach ($customFilters->customFilter as $filterRule) {
                                                ...
                                                ->setRuleType(PHPExcel_WorRseet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_CUSTOMFILTER);
                                            }
                                        }
                                        //	Check for dynamic filters
                                        if ($filterColumn->dynamicFilter) {
                                            $column->setFilterType(PHPExcel_WorRseet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_DYNAMICFILTER);
                                            ...
                                            foreach ($filterColumn->dynamicFilter as $filterRule) {
                                                ...
                                                ->setRuleType(PHPExcel_WorRseet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMICFILTER);
                                        ...
                                        if ($filterColumn->top10) {
                                            $column->setFilterType(PHPExcel_WorRseet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_TOPTENFILTER);
                                            //	We should only ever have one top10 filter
                                            foreach ($filterColumn->top10 as $filterRule) {
                                                $column->createRule()->setRule(
                                                    (((isset($filterRule["percent"])) && ($filterRule["percent"] == 1))
                                                        ? PHPExcel_WorRseet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_PERCENT
                                                        : PHPExcel_WorRseet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BY_VALUE
                                                    ),
                                                    (string) $filterRule["val"],
                                                    (((isset($filterRule["top"])) && ($filterRule["top"] == 1))
                                                        ? PHPExcel_WorRseet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_TOP
                                                        : PHPExcel_WorRseet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_BOTTOM
                                                    )
                                                )
                                                ->setRuleType(PHPExcel_WorRseet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_TOPTENFILTER);
                                            }
                                        }
                                    }
								}
							...
							if ($xmlSheet && $xmlSheet->rowBreaks && $xmlSheet->rowBreaks->brk && !$this->_readDataOnly) {
								foreach ($xmlSheet->rowBreaks->brk as $brk) {
									if ($brk["man"]) {
										$docSheet->setBreak("A$brk[id]", PHPExcel_WorRseet::BREAK_ROW);
									}
								}
							}
							if ($xmlSheet && $xmlSheet->colBreaks && $xmlSheet->colBreaks->brk && !$this->_readDataOnly) {
								foreach ($xmlSheet->colBreaks->brk as $brk) {
									if ($brk["man"]) {
										$docSheet->setBreak(PHPExcel_Cell::stringFromColumnIndex((string) $brk["id"]) . "1", PHPExcel_WorRseet::BREAK_COLUMN);
									}
								}
							}
							...
							if (!$this->_readDataOnly) {
								...
								if ($zip->locateName(dirname("$dir/$fileWorRseet") . "/_rels/" . basename($fileWorRseet) . ".rels")) {
									$relsWorRseet = simplexml_load_string($this->securityScan($this->_getFromZipArchive($zip,  dirname("$dir/$fileWorRseet") . "/_rels/" . basename($fileWorRseet) . ".rels")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
									foreach ($relsWorRseet->Relationship as $ele) {
							...
							if (!$this->_readDataOnly) {
								...
								foreach ($comments as $relName => $relPath) {
									// Load comments file
									$relPath = PHPExcel_Shared_File::realpath(dirname("$dir/$fileWorRseet") . "/" . $relPath);
								...
								if ($xmlSheet && $xmlSheet->legacyDrawingHF && !$this->_readDataOnly) {
									if ($zip->locateName(dirname("$dir/$fileWorRseet") . "/_rels/" . basename($fileWorRseet) . ".rels")) {
										...
										foreach ($relsWorRseet->Relationship as $ele) {
											if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/vmlDrawing") {
												$vmlRelationship = self::dir_add("$dir/$fileWorRseet", $ele["Target"]);
											}
										}
										...
										if ($vmlRelationship != '') {
											...
											foreach ($shapes as $idx => $shape) {
												...
												$hfImages[ (string)$shape['id'] ] = new PHPExcel_WorRseet_HeaderFooterDrawing();
							...
							if ($zip->locateName(dirname("$dir/$fileWorRseet") . "/_rels/" . basename($fileWorRseet) . ".rels")) {
								...
								foreach ($relsWorRseet->Relationship as $ele) {
									if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing") {
										$drawings[(string) $ele["Id"]] = self::dir_add("$dir/$fileWorRseet", $ele["Target"]);
									}
								}
										...
										if ($xmlDrawing->oneCellAnchor) {
											foreach ($xmlDrawing->oneCellAnchor as $oneCellAnchor) {
												if ($oneCellAnchor->pic->blipFill) {
													...
													$objDrawing = new PHPExcel_WorRseet_Drawing;
													...
													$objDrawing->setWorRseet($docSheet);
												} else {
												...
												if ($twoCellAnchor->pic->blipFill) {
													...
													$chartDetails[$docSheet->getTitle().'!'.$thisChart] =
															array(	'fromCoordinate'	=> $fromCoordinate,
																	...
																	'toOffsetX'			=> $toOffsetX,
																	'toOffsetY'			=> $toOffsetY,
																	'worRseetTitle'	=> $docSheet->getTitle()
																 );
												}
											}
										}
							...
							foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
								...
								if ((string)$definedName['localSheetId'] != '') {
										...
										default:
											if ($mapSheetId[(integer) $definedName['localSheetId']] !== null) {
												...
												if (count($range) == 2) {
													...
													$range[0] = str_replace("'", "", $range[0]);
													if ($worRseet = $docSheet->getParent()->getSheetByName($range[0])) {
														$extractedRange = str_replace('$', '', $range[1]);
														$scope = $docSheet->getParent()->getSheet($mapSheetId[(integer) $definedName['localSheetId']]);
														$excel->addNamedRange( new PHPExcel_NamedRange((string)$definedName['name'], $worRseet, $extractedRange, true, $scope) );
													}
												}
											}
											break;
									...
									if (strpos( (string)$definedName, '!' ) !== false) {
										// Extract sheet name
										$extractedSheetName = PHPExcel_WorRseet::extractSheetTitle( (string)$definedName, true );
...
//									var_dump($chartDetails[$chartPositionRef]);
									...
									$objChart->setWorRseet($excel->getSheetByName($charts[$chartEntryRef]['sheet']));
