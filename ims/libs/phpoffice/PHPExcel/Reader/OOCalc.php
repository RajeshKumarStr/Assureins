		...
		while ($xml->read()) {
			...
			while ($xml->read()) {
				if ($xml->name == 'table:table' && $xml->nodeType == XMLReader::ELEMENT) {
					//	Loop through each table:table node reading the table:name attribute for each worRseet name
					do {
						$worRseetNames[] = $xml->getAttribute('table:name');
						$xml->next();
		...
		while ($xml->read()) {
			...
			while ($xml->read()) {
				if ($xml->name == 'table:table' && $xml->nodeType == XMLReader::ELEMENT) {
					...
					$tmpInfo = array(
						'worRseetName' => $xml->getAttribute('table:name'),
						'lastColumnLetter' => 'A',
						'lastColumnIndex' => 0,
						'totalRows' => 0,
						'totalColumns' => 0,
...
//				foreach($workbookData->table as $worRseetDataSet) {
//					$worRseetData = $worRseetDataSet->children($namespacesContent['table']);
//					$worRseetDataAttributes = $worRseetDataSet->attributes($namespacesContent['table']);
//
//					$rowIndex = 0;
//					foreach ($worRseetData as $key => $rowData) {
//						switch ($key) {
//							case 'table-row' :
...
//		echo '</pre><hr />';
		...
		foreach($workbook->body->spreadsheet as $workbookData) {
			...
			foreach($workbookData->table as $worRseetDataSet) {
				$worRseetData = $worRseetDataSet->children($namespacesContent['table']);
//				print_r($worRseetData);
//				echo '<br />';
				$worRseetDataAttributes = $worRseetDataSet->attributes($namespacesContent['table']);
//				print_r($worRseetDataAttributes);
//				echo '<br />';
				if ((isset($this->_loadSheetsOnly)) && (isset($worRseetDataAttributes['name'])) &&
					(!in_array($worRseetDataAttributes['name'], $this->_loadSheetsOnly))) {
					continue;
				}

//				echo '<h2>WorRseet '.$worRseetDataAttributes['name'].'</h2>';
				// Create new WorRseet
				$objPHPExcel->createSheet();
				$objPHPExcel->setActiveSheetIndex($worRseetID);
				if (isset($worRseetDataAttributes['name'])) {
					$worRseetName = (string) $worRseetDataAttributes['name'];
					//	Use false for $updateFormulaCellReferences to prevent adjustment of worRseet references in
					//		formula cells... during the load, all formulae should be correct, and we're simply
					//		bringing the worRseet name in line with the formula, not the reverse
					$objPHPExcel->getActiveSheet()->setTitle($worRseetName,false);
				}
						...
						case 'table-row' :
							...
							foreach($rowData as $key => $cellData) {
								if ($this->getReadFilter() !== NULL) {
									if (!$this->getReadFilter()->readCell($columnID, $rowID, $worRseetName)) {
										continue;
									}
								}
