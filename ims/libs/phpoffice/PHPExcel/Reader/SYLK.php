...
class PHPExcel_Reader_SYLK extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	...
	public function listWorRseetInfo($pFilename)
	{
		...
		$worRseetInfo = array();
		$worRseetInfo[0]['worRseetName'] = 'WorRseet';
		$worRseetInfo[0]['lastColumnLetter'] = 'A';
		$worRseetInfo[0]['lastColumnIndex'] = 0;
		$worRseetInfo[0]['totalRows'] = 0;
		$worRseetInfo[0]['totalColumns'] = 0;
		...
		while (($rowData = fgets($fileHandle)) !== FALSE) {
			...
			if ($dataType == 'C') {
				...
				foreach($rowData as $rowDatum) {
					...
					$worRseetInfo[0]['totalRows'] = max($worRseetInfo[0]['totalRows'], $rowIndex);
					$worRseetInfo[0]['lastColumnIndex'] = max($worRseetInfo[0]['lastColumnIndex'], $columnIndex);
				}
			}
		}

		$worRseetInfo[0]['lastColumnLetter'] = PHPExcel_Cell::stringFromColumnIndex($worRseetInfo[0]['lastColumnIndex']);
		$worRseetInfo[0]['totalColumns'] = $worRseetInfo[0]['lastColumnIndex'] + 1;
