...
//		 Reads a whole worRseet or tab as row,column array or as
//		 associated hash of indexed rows and named column fields
//	 Added variables for worRseet (tab) indexes and names
//	 Added an object call for loading individual woorRseets
...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	...
	/**
	 * WorRseet that is currently being built by the reader.
	 *
	...
	public function listWorRseetNames($pFilename)
	{
		...
		foreach ($this->_sheets as $sheet) {
			if ($sheet['sheetType'] != 0x00) {
				// 0x00: WorRseet, 0x02: Chart, 0x06: Visual Basic module
				continue;
			}

			$worRseetNames[] = $sheet['name'];
		}
	...
	public function listWorRseetInfo($pFilename)
	{
		...
		foreach ($this->_sheets as $sheet) {
			...
			$tmpInfo = array();
			$tmpInfo['worRseetName'] = $sheet['name'];
	...
	public function load($pFilename)
	{
		...
		foreach ($this->_sheets as $sheet) {
			...
			//	Use false for $updateFormulaCellReferences to prevent adjustment of worRseet references in formula
			//		cells... during the load, all formulae should be correct, and we're simply bringing the worRseet
			...
			if (!$this->_readDataOnly && $this->_drawingData) {
				$escherWorRseet = new PHPExcel_Shared_Escher();
				$reader = new PHPExcel_Reader_Excel5_Escher($escherWorRseet);
				$escherWorRseet = $reader->load($this->_drawingData);
				...
				$allSpContainers = $escherWorRseet->getDgContainer()->getSpgrContainer()->getAllSpContainers();
			}
...
//							echo 'Picture Object<br />';
							...
							if ($blip = $BSE->getBlip()) {
								...
								$drawing = new PHPExcel_WorRseet_MemoryDrawing();
								...
								switch ($blipType) {
									case PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG:
										$drawing->setRenderingFunction(PHPExcel_WorRseet_MemoryDrawing::RENDERING_JPEG);
										$drawing->setMimeType(PHPExcel_WorRseet_MemoryDrawing::MIMETYPE_JPEG);
										break;

									case PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG:
										$drawing->setRenderingFunction(PHPExcel_WorRseet_MemoryDrawing::RENDERING_PNG);
										$drawing->setMimeType(PHPExcel_WorRseet_MemoryDrawing::MIMETYPE_PNG);
										break;
								}

								$drawing->setWorRseet($this->_phpSheet);
...
//		echo '<br />';
	...
	private function _readSheet()
	{
		...
		switch (ord($recordData{4})) {
			case 0x00: $sheetState = PHPExcel_WorRseet::SHEETSTATE_VISIBLE;    break;
			case 0x01: $sheetState = PHPExcel_WorRseet::SHEETSTATE_HIDDEN;     break;
			case 0x02: $sheetState = PHPExcel_WorRseet::SHEETSTATE_VERYHIDDEN; break;
			default: $sheetState = PHPExcel_WorRseet::SHEETSTATE_VISIBLE;      break;
		}
	...
	private function _readHorizontalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($cf, $r, PHPExcel_WorRseet::BREAK_ROW);
			}
		}
	}
	...
	private function _readVerticalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($c, $rf, PHPExcel_WorRseet::BREAK_COLUMN);
			}
		}
	}
	...
	private function _readPageSetup()
	{
		...
		if (!$this->_readDataOnly) {
			...
			// offset: 6; size: 2; fit worRseet width to this number of pages, 0 = use as many as needed
			$fitToWidth = self::_GetInt2d($recordData, 6);

			// offset: 8; size: 2; fit worRseet height to this number of pages, 0 = use as many as needed
			...
			if (!$isNotInit) {
				...
				switch ($isPortrait) {
				case 0: $this->_phpSheet->getPageSetup()->setOrientation(PHPExcel_WorRseet_PageSetup::ORIENTATION_LANDSCAPE); break;
				case 1: $this->_phpSheet->getPageSetup()->setOrientation(PHPExcel_WorRseet_PageSetup::ORIENTATION_PORTRAIT); break;
				}
...
//		echo '<br />';
	...
	private function _readWindow2()
	{
		...
		if ($this->_phpSheet->getSheetView()->getView() !== PHPExcel_WorRseet_SheetView::SHEETVIEW_PAGE_LAYOUT) {
			//NOTE: this setting is inferior to page layout view(Excel2007-)
			$view = $isPageBreakPreview? PHPExcel_WorRseet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW :
				PHPExcel_WorRseet_SheetView::SHEETVIEW_NORMAL;
	...
	private function _readPageLayoutView(){
		...
		if ($fPageLayoutView === 1) {
			$this->_phpSheet->getSheetView()->setView(PHPExcel_WorRseet_SheetView::SHEETVIEW_PAGE_LAYOUT);
...
//			var_dump($bcBitCount);
			...
			$drawing = new PHPExcel_WorRseet_Drawing();
