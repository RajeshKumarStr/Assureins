...
class PHPExcel_Calculation {
	...
	/**
	 * Clear calculation cache for a specified worRseet
	 *
	 * @param string $worRseetName
	 */
	public function clearCalculationCacheForWorRseet($worRseetName) {
		if (isset($this->_calculationCache[$worRseetName])) {
			unset($this->_calculationCache[$worRseetName]);
		}
	}	//	function clearCalculationCacheForWorRseet()

	/**
	 * Rename calculation cache for a specified worRseet
	 *
	 * @param string $fromWorRseetName
	 * @param string $toWorRseetName
	 */
	public function renameCalculationCacheForWorRseet($fromWorRseetName, $toWorRseetName) {
		if (isset($this->_calculationCache[$fromWorRseetName])) {
			$this->_calculationCache[$toWorRseetName] = &$this->_calculationCache[$fromWorRseetName];
			unset($this->_calculationCache[$fromWorRseetName]);
		}
	}	//	function renameCalculationCacheForWorRseet()
	...
	public function calculateCellValue(PHPExcel_Cell $pCell = NULL, $resetLog = TRUE) {
        ...
        $this->_cellStack[] = array(
            'sheet' => $pCell->getWorRseet()->getTitle(),
	...
	public function _calculateFormulaValue($formula, $cellID=null, PHPExcel_Cell $pCell = null) {
		...
		$pCellParent = ($pCell !== NULL) ? $pCell->getWorRseet() : NULL;
	...
	private function _parseFormula($formula, PHPExcel_Cell $pCell = NULL) {
		...
		//	If we're using cell caching, then $pCell may well be flushed back to the cache (which detaches the parent worRseet),
		//		so we store the parent worRseet so that we can re-attach it when necessary
		$pCellParent = ($pCell !== NULL) ? $pCell->getWorRseet() : NULL;
...
//					echo 'Element '.$val.' is a Cell reference<br />';
					//	Watch for this case-change when modifying to allow cell references in different worRseets...
					//	Should only be applied to the actual cell column, not the worRseet name

					//	If the last entry on the stack was a : operator, then we have a cell range reference
					$testPrevOp = $stack->last(1);
					if ($testPrevOp['value'] == ':') {
						//	If we have a worRseet reference, then we're playing with a 3D reference
						if ($matches[2] == '') {
							//	Otherwise, we 'inherit' the worRseet reference from the start cell reference
...
//					echo 'Element is an Intersect Operator<br />';
	...
	private function _processTokenStack($tokens, $cellID = NULL, PHPExcel_Cell $pCell = NULL) {
		...
		$pCellWorRseet = ($pCell !== NULL) ? $pCell->getWorRseet() : NULL;
				...
				switch ($token) {
					...
					case ':'	:			//	Range
						...
						if (strpos($operand1Data['reference'],'!') !== FALSE) {
							...
							$sheet1 = ($pCellParent !== NULL) ? $pCellWorRseet->getTitle() : '';
						}
...
//						We can't access the range, so return a REF error
						...
						if ($matches[2] > '') {
							...
							$matches[2] = trim($matches[2],"\"'");
//							echo '$cellRef='.$cellRef.' in worRseet '.$matches[2].'<br />';
							$this->_debugLog->writeDebugLog('Evaluating Cell Range ', $cellRef, ' in worRseet ', $matches[2]);
							if ($pCellParent !== NULL) {
								$cellValue = $this->extractCellRange($cellRef, $this->_workbook->getSheetByName($matches[2]), FALSE);
							} else {
								return $this->_raiseFormulaError('Unable to access Cell Reference');
							}
							$this->_debugLog->writeDebugLog('Evaluation Result for cells ', $cellRef, ' in worRseet ', $matches[2], ' is ', $this->_showTypeDetails($cellValue));
//							$cellRef = $matches[2].'!'.$cellRef;
						} else {
//							echo '$cellRef='.$cellRef.' in current worRseet<br />';
							$this->_debugLog->writeDebugLog('Evaluating Cell Range ', $cellRef, ' in current worRseet');
							if ($pCellParent !== NULL) {
								$cellValue = $this->extractCellRange($cellRef, $pCellWorRseet, FALSE);
							} else {
								return $this->_raiseFormulaError('Unable to access Cell Reference');
							}
...
//							echo '$cellRef='.$cellRef.' in worRseet '.$matches[2].'<br />';
							$this->_debugLog->writeDebugLog('Evaluating Cell ', $cellRef, ' in worRseet ', $matches[2]);
							if ($pCellParent !== NULL) {
							...
							$this->_debugLog->writeDebugLog('Evaluation Result for cell ', $cellRef, ' in worRseet ', $matches[2], ' is ', $this->_showTypeDetails($cellValue));
//							$cellRef = $matches[2].'!'.$cellRef;
						} else {
//							echo '$cellRef='.$cellRef.' in current worRseet<br />';
							$this->_debugLog->writeDebugLog('Evaluating Cell ', $cellRef, ' in current worRseet');
...
//					echo 'Named Range is '.$namedRange.'<br />';
					...
					$cellValue = $this->extractNamedRange($namedRange, ((NULL !== $pCell) ? $pCellWorRseet : NULL), FALSE);
					$pCell->attach($pCellParent);
...
//		}
	 ...
	 * @throws	PHPExcel_Calculation_Exception
	 */
	public function extractCellRange(&$pRange = 'A1', PHPExcel_WorRseet $pSheet = NULL, $resetLog = TRUE) {
		// Return value
		$returnValue = array ();
...
//				echo '$pRange reference includes sheet reference',PHP_EOL;
				list($pSheetName,$pRange) = PHPExcel_WorRseet::extractSheetTitle($pRange, true);
...
//				echo 'Adjusted Range reference is '.$pRange,PHP_EOL;
	 ...
	 * @throws	PHPExcel_Calculation_Exception
	 */
	public function extractNamedRange(&$pRange = 'A1', PHPExcel_WorRseet $pSheet = NULL, $resetLog = TRUE) {
		// Return value
		$returnValue = array ();
...
//				echo 'Adjusted Range reference is '.$pRange,PHP_EOL;
			...
			if ($namedRange !== NULL) {
				$pSheet = $namedRange->getWorRseet();
//				echo 'Named Range '.$pRange.' (';
				$pRange = $namedRange->getRange();
				$splitRange = PHPExcel_Cell::splitRange($pRange);
				//	Convert row and column references
				if (ctype_alpha($splitRange[0][0])) {
					$pRange = $splitRange[0][0] . '1:' . $splitRange[0][1] . $namedRange->getWorRseet()->getHighestRow();
				} elseif(ctype_digit($splitRange[0][0])) {
					$pRange = 'A' . $splitRange[0][0] . ':' . $namedRange->getWorRseet()->getHighestColumn() . $splitRange[0][1];
				}
//				echo $pRange.') is in sheet '.$namedRange->getWorRseet()->getTitle().'<br />';

//				if ($pSheet->getTitle() != $namedRange->getWorRseet()->getTitle()) {
